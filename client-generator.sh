...
kubeclient::generator::generate_client() {
    ...
    docker build --rm --force-rm "${SCRIPT_ROOT}"/../ -f "${SCRIPT_ROOT}/Dockerfile" --network host --pull -t "${image_name}" \
	--build-arg HTTP_PROXY="http://127.0.0.1:7777" \
	--build-arg HTTPS_PROXY="http://127.0.0.1:7777" \
	--build-arg http_proxy="http://127.0.0.1:7777" \
	--build-arg https_proxy="http://127.0.0.1:7777" \
